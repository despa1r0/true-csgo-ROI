...
Основной сценарий: ввод названия или выбор коллекции → выбор скина → выбор режима Raw/Smart/Enhanced/Quick flip, направления покупки/продажи и способов пополнения/вывода → сравнение цен и ROI CSFloat/CSGO Market на карточках FN/MW/FT/WW/BS → выбор площадки и модальный список её лотов → параллельная аналитика обеих площадок. Quick flip автоматически берёт самый дешёвый ask и лениво загружает лучший bid другой площадки; остальные режимы дополнительных приватных запросов не делают. Приватный Market.CSGO API защищён общим thread-safe limiter с безопасным потолком четыре запроса в секунду.
