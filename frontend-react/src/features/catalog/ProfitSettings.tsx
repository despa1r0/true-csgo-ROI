...
export function ProfitSettings({ value, marketplaces, onChange }: Props) {
  ...
  return (
    <section className={styles.profitBar} aria-label={t("profit.settings")}>
      ...
      <div className={styles.profitControls}>
        <label className={styles.profitMode}><span>{t("profit.mode")}</span><select name="profit-mode" value={value.mode} onChange={(event) => patch({ mode: event.target.value as ProfitSettingsValue["mode"] })}>{modes.map((mode) => <option key={mode} value={mode}>{t(`profit.${mode}`)}</option>)}</select></label>
        <div className={styles.profitRoute}>
          <label><span>{t("profit.buyOn")}</span><select name="buy-marketplace" disabled={value.mode === "quick_flip"} value={value.buyMarketplace} onChange={(event) => { const nextId = event.target.value; const nextMarket = marketplaces.find((item) => item.id === nextId); const nextDeposit = nextId === "all" ? commonMethods("deposit_methods") : nextMarket?.deposit_methods ?? []; patch({ buyMarketplace: nextId, depositMethod: nextDeposit.includes(value.depositMethod) ? value.depositMethod : nextDeposit[0] ?? "crypto", sellMarketplace: nextId === "all" ? "auto" : value.sellMarketplace === nextId ? enabledMarkets.find((item) => item.id !== nextId)?.id ?? "auto" : value.sellMarketplace }); }}><option value="all">{t("common.all")}</option>{enabledMarkets.map((market) => <option key={market.id} value={market.id}>{market.display_name}</option>)}</select></label>
          ...
          <label><span>{t("profit.sellOn")}</span><select name="sell-marketplace" disabled={automatic || value.mode === "quick_flip"} value={automatic ? "auto" : value.sellMarketplace} onChange={(event) => { const nextId = event.target.value; const methods = nextId === "auto" ? commonMethods("withdraw_methods") : marketplaces.find((item) => item.id === nextId)?.withdraw_methods ?? []; patch({ sellMarketplace: nextId, withdrawMethod: methods.includes(value.withdrawMethod) ? value.withdrawMethod : methods[0] ?? "crypto" }); }}><option value="auto">{t("profit.auto")}</option>{enabledMarkets.map((market) => <option key={market.id} value={market.id}>{market.display_name}</option>)}</select></label>
        </div>
        <label><span>{t("profit.deposit")}</span><select name="deposit-method" disabled={value.mode === "raw" || value.mode === "enhanced"} value={depositMethods.includes(value.depositMethod) ? value.depositMethod : depositMethods[0]} onChange={(event) => patch({ depositMethod: event.target.value as PaymentMethod })}>{depositMethods.map((method) => <option value={method} key={method}>{t(`profit.${method}`)}</option>)}</select></label>
        <label><span>{t("profit.withdraw")}</span><select name="withdraw-method" disabled={value.mode === "raw"} value={withdrawMethods.includes(value.withdrawMethod) ? value.withdrawMethod : withdrawMethods[0]} onChange={(event) => patch({ withdrawMethod: event.target.value as PaymentMethod })}>{withdrawMethods.map((method) => <option value={method} key={method}>{t(`profit.${method}`)}</option>)}</select></label>
        {value.mode === "quick_flip" && <label className={styles.inlineCheck}><input name="include-deposit-fee" type="checkbox" checked={value.useDepositFee} onChange={(event) => patch({ useDepositFee: event.target.checked })} />{t("profit.includeDeposit")}</label>}
      </div>
    </section>
  );
}
